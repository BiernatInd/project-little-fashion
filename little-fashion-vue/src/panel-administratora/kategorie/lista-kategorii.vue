    ...
    <table>
      <thead>
          <tr>
            ...
            <th>Ścieżka kategorii</th>
            <th>Szczegóły</th>
          </tr>
      </thead>
      <tbody>
          <tr v-for="kategorie in kategorie" :key="kategorie.id">
              <td data-label="Numer">{{ kategorie.id }}</td>
              <td data-label="Nazwa kategorii">{{ kategorie.nazwa_kategorii }}</td>
              <td data-label="Ścieżka kategorii">{{ kategorie.sciezka_kategorii}}</td>
              <td data-label="Szczegóły">
              <svg width="16" height="4" viewBox="0 0 16 4" fill="none" xmlns="http://www.w3.org/2000/svg"  @click="redirectToEditPage(kategorie.id)">
              ...
              <svg width="21" height="22" viewBox="0 0 21 22" fill="none" xmlns="http://www.w3.org/2000/svg" @click="showConfirmation(kategorie.id)">
    ...
    <div class="confirmation-content">
      <div class="confirmation-main">
          ...
          <div class="confirmation-bottom">
              <p>Czy napewno chcesz usunąć tą kategorię?</p>
              <div class="confirmation-buttons">
                <button
      class="confirmation-btn"
      :class="{ 'active': confirmationActive }"
      @click="hideConfirmation"
      ref="collection"
    >
      Anuluj
    </button>
    <button class="confirmation-btn" @click="deleteKategorie">
      Usuń
    </button>
              </div>
          </div>
...
methods: {
  redirectToEditPage(kategorieId) {
      // Tutaj następuje programowe przekierowanie do strony edycji
      this.$router.push(`/edytowanie-kategorii/${kategorieId}`);
    },
    showConfirmation(kategorieId) {
      console.log('showConfirmation');
      this.confirmationActive = !this.confirmationActive;
      this.selectedKategorieId = kategorieId; // Dodaj zmienną do przechowywania ID wybranego produktu
  ...
  fetchData() {
  // Pobierz dane z API
  axios.get('http://localhost:8000/api/lista-kategorii')
    .then(response => {
...
deleteKategorie() {
      ...
      axios
        .delete(`http://localhost:8000/api/usun-kategorie/${this.selectedKategorieId}`)
        .then(response => {
