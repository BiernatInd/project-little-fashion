            ...
            <div class="home-left">
                <div class="home-left-box">
                    <div class="home-box-content">
                        <h2>Edytowanie nagłówków</h2>
                        <div class="input-flex">
                            <input v-model="numerNaglowka" type="text" placeholder="Wprowadź numer nagłówka">
                            <input v-model="nowyNaglowek" type="text" placeholder="Wprowadź nowy nagłówek">
                        </div>
                        <div class="home-button">
                            <button @click="edytujNaglowek" class="home-btn">Edytuj nagłówek</button>
                        </div>
                    </div>
                </div>
                <div class="home-left-box">
    <div class="home-box-content">
      <h2>Edytowanie paragrafów</h2>
        <input v-model="numerParagrafu" type="text" placeholder="Wprowadź numer paragrafu">
        <textarea v-model="nowyParagraf" placeholder="Wprowadź nowy paragraf"></textarea>
      <div class="home-button">
        <button @click="edytujParagraf" class="home-btn">Edytuj paragraf</button>
      </div>
    </div>
  </div>
            </div>
...
export default {
    ...
    methods: {
        edytujNaglowek() {
            if (!this.numerNaglowka.trim() || !this.nowyNaglowek.trim()) {
        // Display an alert if the header number or new header field is empty
        alert('Wszystkie pola muszą zostać wypełnione.');
        return;
      }

  axios
    .put(`http://localhost:8000/api/edytuj-naglowek`, {
      numerNaglowka: this.numerNaglowka,
      nowyNaglowek: this.nowyNaglowek,
    })
        .then((response) => {
          // Obsłuż odpowiedź po udanej aktualizacji nagłówka
          console.log("Nagłówek zaktualizowany pomyślnie");
          // Dodatkowo, możesz ponownie pobrać dane, aby zaktualizować widok
          this.pobierzDane();
        })
        .catch((error) => {
          console.error("Błąd podczas aktualizacji nagłówka:", error);
        });
    },
    edytujParagraf() {
        if (!this.numerParagrafu.trim() || !this.nowyParagraf.trim()) {
        // Display an alert if the paragraph number or new paragraph field is empty
        alert('Wszystkie pola muszą zostać wypełnione.');
        return;
      }

      axios
        .put(`http://localhost:8000/api/edytuj-paragraf`, {
          numerParagrafu: this.numerParagrafu,
          nowyParagraf: this.nowyParagraf,
        })
        .then((response) => {
          console.log("Paragraf zaktualizowany pomyślnie");
          this.pobierzDane(); // Optionally update the view by fetching data again
        })
        .catch((error) => {
          console.error("Błąd podczas aktualizacji paragrafu:", error);
        });
    },
        getHeaderType(index) {
