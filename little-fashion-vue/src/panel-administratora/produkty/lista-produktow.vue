<template>
      ...
      <table>
        ...
        <tbody>
            <tr v-for="product in products" :key="product.id">
                ...
                <td data-label="Nazwa produktu">{{ product.nazwa }}</td>
                <td data-label="Cena">{{ product.cena }}</td>
                <td data-label="Stan magazynowy">{{ product.stan_magazynowy }}</td>
    ...
    <button class="confirmation-btn" @click="deleteProduct">
      Usuń
    </button>
                </div>
            </div>
...
export default {
  ...
  methods: {
    redirectToEditPage(productId) {
      ...
      this.$router.push(`/edytowanie-produktow/${productId}`);
    },
    ...
    deleteProduct() {
      ...
      axios
        .delete(`http://localhost:8000/api/usun-produkt/${this.selectedProductId}`)
        .then(response => {
    ...
    fetchData() {
  // Pobierz dane z API
  axios.get('http://localhost:8000/api/lista-produktow')
    .then(response => {
