<template>
    ...
    <div class="home">
      <table>
        ...
        <tbody>
            <tr v-for="rabat in rabaty" :key="rabat.id">
  <td data-label="Numer">{{ rabat.id }}</td>
  <td data-label="Kod rabatowy">{{ rabat.kod_rabatowy }}</td>
  <td data-label="Wartość kodu rabatowego">{{ rabat.znizka }}</td>
            <td data-label="Szczegóły">
                <svg width="21" height="22" viewBox="0 0 21 22" fill="none" xmlns="http://www.w3.org/2000/svg" @click="showConfirmation(rabat.id)">
...
export default {
  ...
  methods: {
    showConfirmation(rabatId) {
      console.log('showConfirmation');
      this.confirmationActive = !this.confirmationActive;
      this.selectedRabatId = rabatId; // Dodaj zmienną do przechowywania ID wybranego produktu
    ...
    fetchData() {
    // Pobierz dane z API
    axios.get('http://localhost:8000/api/pobierz-kody-rabatowe')
      .then(response => {
        console.log('Odpowiedź z API:', response.data);
        // Poprawione przypisanie do pola rabaty
        this.rabaty = response.data.rabaty;
      })
      .catch(error => {
  ...
  deleteRabat() {
      ...
      axios
        .delete(`http://localhost:8000/api/usun-rabat/${this.selectedRabatId}`)
        .then(response => {
