    ...
    <div class="home">
      <table>
        <thead>
            <tr>
              <th>Numer</th>
              <th>Ścieżka kolekcji</th>
              <th>Data</th>
              <th>Szczegóły</th>
            </tr>
        </thead>
        <tbody>
            <tr v-for="(entry, index) in collectionData" :key="index">
          <td data-label="Numer kolekcji">{{ entry.id }}</td>
          <td data-label="Ilość zdjęć w kolekcji">{{ entry.sciezka_kolekcji }}</td>
          <td data-label="Numer kolumny w pionie">{{ formatData(entry.created_at) }}</td>
                <td data-label="Szczegóły">
                <svg width="21" height="22" viewBox="0 0 21 22" fill="none" xmlns="http://www.w3.org/2000/svg"  @click="showConfirmation(entry.id)">
...
export default {
  ...
  data() {
    return {
      ...
      collectionData: [],
      sciezkaKolekcji: null, // Dodaj zmienną do kontekstu komponentu
    }
  },
  methods: {
    redirectToCollection() {
      // Tutaj użyj Vue Router do przekierowania użytkownika
      // Przykładowy kod zakłada, że masz zdefiniowaną ścieżkę "/lista-tresci-kolekcji/:sciezkaKolekcji" w routerze
      this.$router.push(`/lista-tresci-kolekcji/${this.sciezkaKolekcji}`);
    },
    showConfirmation(collectionId) {
        console.log('showConfirmation');
        this.confirmationActive = !this.confirmationActive;
        this.collectionIdToDelete = collectionId; // Dodaj nową właściwość
    ...
    usunTresciKolekcji() {
        axios.delete(`http://localhost:8000/api/usun-tresci-kolekcji/${this.collectionIdToDelete}`)
            .then(response => {
                // Tutaj możesz obsłużyć odpowiedź z API po usunięciu kolekcji
                console.log(response.data);
                // Odśwież dane kolekcji po usunięciu
                this.collectionData = this.collectionData.filter(entry => entry.id !== this.collectionIdToDelete);
                // Ukryj potwierdzenie
  ...
  mounted() {
    // Pobranie sciezkaKolekcji z parametrów routingu
    this.sciezkaKolekcji = this.$route.params.sciezkaKolekcji;

    // Wywołanie API po załadowaniu komponentu
    axios.get(`http://localhost:8000/api/pobierz-dane-tresci-kolekcji/${this.sciezkaKolekcji}`)
      .then(response => {
        // Tutaj możesz obsłużyć odpowiedź z API
        this.collectionData = response.data; // Ustawienie danych w właściwości
        console.log(response.data);
      })
      .catch(error => {
        // Tutaj możesz obsłużyć ewentualne błędy
