      ...
      <table>
        ...
        <tbody>
          ...
          <td data-label="Numer kolumny w pionie">{{ formatData(entry.created_at) }}</td>
                ...
                <svg width="16" height="4" viewBox="0 0 16 4" fill="none" xmlns="http://www.w3.org/2000/svg" @click="redirectToCollection(entry.sciezka_kolekcji)">
...
export default {
  ...
  methods: {
    redirectToCollection(sciezkaKolekcji) {
      ...
      this.$router.push(`/lista-tresci-kolekcji/${sciezkaKolekcji}`);
    },
    ...
    formatData(isoDate) {
        ...
        usunKolekcje() {
        axios.delete(`http://localhost:8000/api/usun-kolekcje/${this.collectionIdToDelete}`)
            .then(response => {
    ...
    mounted() {
        ...
        .then(response => {
            ...
            this.collectionData = response.data; // Set the data to your property
        console.log(response.data);
      })
      .catch(error => {
          // Tutaj możesz obsłużyć ewentualne błędy
